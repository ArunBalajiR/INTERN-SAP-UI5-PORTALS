sap.ui.define([
	...
	return Controller.extend("PortalPortal.controller.ProductionOrderYear", {
		...
		onFilterProdYear: function(oEvent) {
			...
			// filter binding
			var oList = this.getView().byId("productionOrderFilterYrTable");
			var oBinding = oList.getBinding("items");
			oBinding.filter(aFilter);

		},
		
		onProductionFilterYrSelect : function(){
			this._getprodOrderDialog().open();
			
			var ooModelData = this.getView().byId("productionOrderFilterYrTable").getModel("productionOrderFilterYr");
			var rr = this.getView().byId("productionOrderFilterYrTable")._aSelectedPaths[0].split("/")[2];
			...
			sap.ui.getCore().byId("pr_TargetQuantity").setText(aaRows.results[rr].TargetQuantity);
			...
			sap.ui.getCore().byId("pr_ActualReleaseDate").setText(aaRows.results[rr].ActualReleaseDate.toLocaleDateString());
			sap.ui.getCore().byId("pr_SystemStatus").setText(aaRows.results[rr].SystemStatus);
			window.console.log(aaRows.results[rr].OrderNumber);
		...
		setFilteredData : function(){
			var jsonModelprodyr = new JSONModel();
			...
			jsonModelprodyr.setData(oDataprodfilter);
		
			this.getView().setModel(jsonModelprodyr, "productionOrderFilterYr");
